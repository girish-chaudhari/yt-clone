  ...
  getManySubscribed: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const data = await db
        ...
        .innerJoin(viewerSubsciptions, eq(viewerSubsciptions.userId, users.id))
        .where(
          and(
            eq(videos.visibiliy, "public"),
            cursor
              ? or(
      ...
      // Set the next cursor to the last item if there is a next page
  ...
  getOne: baseProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const [existingVideo] = await db
        .with(viewerReactions, viewerSubsciptions)
        .select({
          ...getTableColumns(videos),
          user: {
